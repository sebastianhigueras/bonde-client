...
export const createServer = (config) => {
  ...
  app.get('*', (req, res) => {
    ...
    // Assets
    const assetsByChunkName = !__PROD__ ? res.locals.webpackStats.toJson().assetsByChunkName : null
          ...
          res.status(200).send(`
            ...
            <html lang="pt-br">
              ...
              <body>
                ...
                <script src="${__PROD__ ? assets.vendor.js : normalizeAssets(assetsByChunkName.vendor).filter(path => path.endsWith('.js')).map(path => `${path}`)}"></script>
                <script async src="${__PROD__ ? assets.main.js : normalizeAssets(assetsByChunkName.main).filter(path => path.endsWith('.js')).map(path => `${path}`)}" ></script>
